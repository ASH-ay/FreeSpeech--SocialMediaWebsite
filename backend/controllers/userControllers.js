...
const loginUser = async (req, res, next) => {
    try {
        ...
        return next(new HttpError("Login Failed. Invalid Email or Password", 422))
    }
}
...
const changeAvatar = async (req, res, next) => {
    try {
        ...
        if (avatar.size > 500000) {
            return next(new HttpError("Picture size should be less than 500kb "), 422)
        }
        ...
        avatar.mv(path.join(__dirname, '..', 'uploads', newFilename), async (err) => {
            ...
            if (!updatedAvatar) {
                return next(new HttpError("Profile Pic could not be uploaded.", 422))
            }
